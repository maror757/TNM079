...
float LevelSet::Diff2Xpm(size_t i, size_t j, size_t k) const { 
	...
	return (mGrid.GetValue(i + 1, j, k) - 2.0f*mGrid.GetValue(i, j, k) + mGrid.GetValue(i - 1, j, k)) 
											/ mDx*mDx; 
}
...
float LevelSet::Diff2Ypm(size_t i, size_t j, size_t k) const { 
	...
	return (mGrid.GetValue(i, j + 1, k) - 2.0f*mGrid.GetValue(i, j, k) + mGrid.GetValue(i, j - 1, k))
											/ mDx*mDx;
}
...
float LevelSet::Diff2Zpm(size_t i, size_t j, size_t k) const {
	...
	return  (mGrid.GetValue(i, j, k + 1) - 2.0f*mGrid.GetValue(i, j, k) + mGrid.GetValue(i, j, k - 1)) 
												/ mDx*mDx;
}
